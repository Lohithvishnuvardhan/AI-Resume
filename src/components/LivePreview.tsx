...
const LivePreview: React.FC<LivePreviewProps> = ({ resumeData, template, templateColorScheme = 0, atsScore, healthScore }) => {
  ...
  const getTemplateStyles = () => {
    ...
    switch (template) {
      ...
      case 'executive-gold':
      case 'startup-founder':
        return `bg-gradient-to-br ${currentScheme.bgGradient} ${currentScheme.border}`;
      case 'tech-minimal':
        return `${currentScheme.bgGradient} border ${currentScheme.border}`;
      case 'modern':
      case 'creative':
        return `bg-gradient-to-br ${currentScheme.bgGradient} ${currentScheme.border}`;
      case 'executive':
      ...
      default:
        return `${currentScheme.bgGradient} ${currentScheme.border}`;
    }
  };
  ...
  const SectionDivider = () => {
    const isDarkTheme = template === 'executive';
    return (
      <div className={`my-5 border-t ${isDarkTheme ? 'border-gray-700' : 'border-gray-200'}`}></div>
    );
  };
              ...
              <div className="space-y-4">
                {resumeData.experience.map((exp, index) => (
                  <div key={index} className={`pl-4 border-l-2 ${currentScheme.border.replace('border-l-4', 'border-l-2')}`}>
                    <h3 className={`font-bold ${currentScheme.header} text-base`}>{exp.position}</h3>
              ...
              <div className="space-y-4">
                {resumeData.projects.map((project, index) => (
                  <div key={index} className={`p-4 rounded-lg border-l-4 ${currentScheme.border} bg-gray-50`}>
                    <h3 className={`font-bold ${currentScheme.header}`}>{project.title}</h3>
            ...
            <div className="space-y-2 text-xs">
              {resumeData.personalInfo.email && (
                ...
                >
                  ...
                  <span className="break-all text-gray-700">{resumeData.personalInfo.email}</span>
                </a>
              )}
              ...
              {resumeData.socialLinks?.linkedin && (
                ...
                >
                  ...
                  <span className="text-gray-700 text-xs break-all">{resumeData.socialLinks.linkedin.replace('https://', '').replace('http://', '')}</span>
                </a>
              )}
              {resumeData.socialLinks?.github && (
                ...
                >
                  ...
                  <span className="text-gray-700 text-xs break-all">{resumeData.socialLinks.github.replace('https://', '').replace('http://', '')}</span>
                </a>
              )}
        ...
        <div className={`pb-5 mb-5 ${template === 'executive' ? 'border-b border-gray-700' : 'border-b border-gray-300'}`}>
          <div className="flex items-start gap-4 sm:gap-6">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className={`flex flex-wrap gap-2 sm:gap-3 text-xs mb-2 ${template === 'executive' ? 'text-gray-400' : 'text-gray-600'}`}>
                {resumeData.personalInfo.email && (
                  <a
                    href={`mailto:${resumeData.personalInfo.email}`}
                    className={`flex items-center transition-colors ${template === 'executive' ? 'hover:text-gray-200' : 'hover:text-blue-600'}`}
                  >
        ...
        {resumeData.personalInfo.summary && (
          <>
            <div className="mb-6">
              ...
              <p className={`${textStyles.bodyText || 'text-gray-700'} leading-relaxed`}>{resumeData.personalInfo.summary}</p>
            </div>
            <SectionDivider />
          </>
        )}
              ...
              <div className="space-y-4">
                {resumeData.experience.map((exp, index) => (
                  <div key={index} className="pl-0">
                    ...
                    {exp.description && <p className={`${textStyles.bodyText || 'text-gray-700'} text-sm leading-relaxed`}>{exp.description}</p>}
                  </div>
                ))}
              </div>
            </div>
          ...
          <>
            <div className="mb-4 sm:mb-5 lg:mb-6">
              ...
              <div className="space-y-3 sm:space-y-4">
                {resumeData.projects.map((project, index) => (
                  <div key={index} className={`p-4 rounded-lg hover:shadow-md transition-shadow ${template === 'executive' ? 'bg-slate-800 bg-opacity-50' : 'bg-gray-50'}`}>
                    ...
                    <p className={`${textStyles.bodyText || 'text-gray-700'} mt-2 text-sm leading-relaxed`}>{project.description}</p>
                  </div>
                ))}
              </div>
            </div>
                ...
                {resumeData.certifications.map((cert, index) => (
                  <div key={index} className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className={`text-sm ${template === 'executive' ? 'text-gray-400' : 'text-gray-600'}`}>{cert.issuer} • {cert.year}</p>
                    </div>
                  </div>
                ))}
              </div>
